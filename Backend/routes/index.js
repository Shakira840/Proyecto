...
const authenticateToken = (req, res, next) => {
  ...
  try{
    ...
    console.error(error);
    res.status(403).json({ error: "Token inválido o expirador"})
  }
}
...
router.patch('/habits/marksdone/:id', authenticateToken, async (req, res) => {
  try{
    ...
    if(timeDifferenceInHours(habit.lastDone, habit.lastUpdate) < 24){
      ...
      await habit.save();
      res.status(200).json({'message': 'Habit market as done'});
    }else{
      habit.days = 1;
      habit.lastUpdate = new Date ();
      habit.save();
